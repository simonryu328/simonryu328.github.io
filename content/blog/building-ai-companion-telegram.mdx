...
Since my FastAPI server also needs to serve the Telegram Mini App frontend (a static HTML/JS/CSS bundle) and expose the REST API endpoints it uses, I utilized FastAPI's lifespan context manager pattern. This essentially boots up the bot's listening service right alongside the API server:
...
Furthermore, I invoke Telegram's typing indicator API before every single message bubble is sent. The duration of the typing status scales proportionally with the character count of the incoming bubble. It’s a tiny detail, but it singlehandedly sells the illusion of physical presence.
