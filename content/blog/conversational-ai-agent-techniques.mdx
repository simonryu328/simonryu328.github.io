...
To fix this, I abandoned free-text output and enforced a strict XML schema for every single response. I chose XML over JSON because I found LLMs are far less likely to break XML formatting when generating creative text, and it's easier to regex-parse a missing close-tag than a missing JSON bracket.
...
Every system prompt I generate injects the user's localized time of day. I store the user's IANA timezone upon registration and convert all timestamps in the prompt to their local time. "It's 2am" and "It's 10am" drastically alter how the agent interprets the same message.
...
### Events vs. Traits (Two Memory Systems)
...
By storing both types of records, the agent can reference a past event while also understanding the emotional weight behind it.
...
I built an asynchronous APScheduler cron job that routinely scans the database for inactive users (e.g., users who haven't sent a message in more than 6 hours). When the system flags an inactive user, it gathers their recent `compact_summary` entries and passes them through a specialized proactive prompt.
...
The agent uses these reach-outs to prove it's actively "thinking" about the user even when the chat window is closed. (Naturally, users can configure the frequency, quiet-hours, or entirely disable these reach-outs via native Telegram commands).
